...
The worker rechecks account/list access and expiry before extraction and persistence, uses the safe outbound transport, and commits source results before running comparisons. Processing and lease renewal use separate scopes/contexts. Each comparison uses its own scope and saves results incrementally. The status endpoint and frontend polling expose partial results at approximately two-second intervals.
...
The existing matching and persistence services reuse the canonical product for matching GTINs across Coles and Woolworths, subject to contradiction checks. Stage 6A adds search providers, and Stage 6B connects the other-retailer check after source persistence. No database migration is required.
...
Stage 6A provided the search components; Stage 6B below connects them to background import jobs. The existing frontend polls their results. No database migration is required.
...
The browser dependency is loaded only when search is invoked. Source extraction and known-mapping refreshes use safe HTTP; an import can now launch the browser when its other-retailer comparison needs search. See the official [Playwright network documentation](https://playwright.dev/dotnet/docs/network) and [browser contexts](https://playwright.dev/dotnet/docs/api/class-browsercontext).
...
## Background retailer comparisons (Stage 6B)

After the source transaction commits, `RetailerComparisonService` checks each other active retailer sequentially in a separate dependency scope. Each result is persisted immediately through `RetailerComparisonPersistenceService`; no database transaction or catalogue lock spans network work. A restart with a saved source resumes comparisons without importing the source or incrementing the list quantity again. Completed retailer results are preserved.

The lookup order is:

1. Reuse one active exact mapping when all its stored observations are sufficiently fresh. `ProductImport:ComparisonFreshHours` defaults to 6 (configurable from 1 to 168 hours; environment variable `ProductImport__ComparisonFreshHours`). Cache reuse requires AUD, no store location, an anonymous/online/national scope, no future check time, and no expired or future promotion. Mixed stale/store/currency observations prevent cache reuse because the existing status API exposes all observations for a mapping. Cache results retain their original check time and set `IsFromCache`; they create no price history.
2. Refresh an existing exact mapping through its known product URL using the safe source extractor, and verify the returned identity and retailer product code. Multiple existing exact mappings are treated as ambiguous.
3. When there is no exact mapping, search and apply the existing deterministic matching rules to the bounded candidate set. Persist only a unique exact match. Multiple exact candidates fail explicitly; a likely candidate reports `Likely` without linking it to the canonical product or exposing its price. `NotFound` with `no_verified_match_in_candidates` means no match among the checked candidates, not proof that the entire retailer catalogue lacks the product.

Persistence checks the current canonical identity again under the catalogue lock. It refuses to move another canonical product's mapping, rolls back invalid offers, preserves current-price/history rules, and checks ownership, trial/list eligibility and the current unexpired job lease both before writing and before commit. Anonymous prices keep `PriceScope.Unknown`; no local-store or cheapest-price claim is added.
